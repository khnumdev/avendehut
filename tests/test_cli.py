...
def test_cli_search_and_export(tmp_path: Path) -> None:
    ...
    writer.add_metadata({"/Title": "Hello", "/Author": "Bob"})
    ...
    result = runner.invoke(
        main, ["export", "--out", str(json_out), "--format", "json", "--src-out", str(out)]
    )
    assert result.exit_code == 0
    ...
    result = runner.invoke(
        main, ["export", "--out", str(csv_out), "--format", "csv", "--src-out", str(out)]
    )
    assert result.exit_code == 0
